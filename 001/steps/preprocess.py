...
def preprocess_chat(s3_output_path):
    dataset_path = 'allenai/sciq'
    dataset = load_dataset(dataset_path)
    ...
    finetuning_dataset_filename = "dataset_finetune_chat.jsonl"
    evaluation_dataset_filename = "dataset_evaluation.jsonl"
    finetuning_dataset_local_path = f"./output/{dataset_path}/{finetuning_dataset_filename}"
    evaluation_dataset_local_path = f"./output/{dataset_path}/{evaluation_dataset_filename}"

    # Create CHAT dataset
    dataset_train_chat_df = dataset_training_df[['question', 'correct_answer']].copy()
    dataset_str = ""
    for index, row in dataset_train_chat_df.iterrows():
        sample = {"dialog": [{"content": row['question'], "role": "user"},
                             {"content": row['correct_answer'], "role": "assistant"}]}
        dataset_str += str(sample) + "\n"

    write_to_file(dataset_str, finetuning_dataset_local_path)
    ...
    finetuning_dataset_s3_path = f"{s3_output_path}/{dataset_path}/finetuning/chat"
    evaluation_dataset_s3_path = f"{s3_output_path}/{dataset_path}/evaluation"
